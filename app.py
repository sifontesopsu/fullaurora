...
def read_full_excel(file) -> pd.DataFrame:
    ...
    for sh in xls.sheet_names:
        ...
        # Fallback específico para el formato nuevo:
        # Area., Nº, Código ML, Código Universal, SKU, Descripción, Unidades, Identificación, Vence, Dia, Hora
        if sku_col is None and len(cols_orig) >= 5:
            sku_col = cols_orig[4]
        if title_col is None and len(cols_orig) >= 6:
            title_col = cols_orig[5]
        if qty_col is None and len(cols_orig) >= 7:
            qty_col = cols_orig[6]
        if area_col is None and len(cols_orig) >= 1:
            area_col = cols_orig[0]
        if nro_col is None and len(cols_orig) >= 2:
            nro_col = cols_orig[1]
        if codigo_ml_col is None and len(cols_orig) >= 3:
            codigo_ml_col = cols_orig[2]
        if codigo_univ_col is None and len(cols_orig) >= 4:
            codigo_univ_col = cols_orig[3]
        if etiquetar_col is None and len(cols_orig) >= 8:
            etiquetar_col = cols_orig[7]
        if vence_col is None and len(cols_orig) >= 9:
            vence_col = cols_orig[8]
        if dia_col is None and len(cols_orig) >= 10:
            dia_col = cols_orig[9]
        if hora_col is None and len(cols_orig) >= 11:
