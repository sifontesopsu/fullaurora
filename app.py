...
def init_db():
    ...
    # FULL: columnas nuevas del formato 23-04-2026
    _ensure_col("full_batch_items", "codigo_ml", "TEXT")
    _ensure_col("full_batch_items", "codigo_universal", "TEXT")
    _ensure_col("full_batch_items", "dia", "TEXT")
    _ensure_col("full_batch_items", "hora", "TEXT")
...
def _norm_header(x) -> str:
    """Normaliza encabezados: acentos, puntos, símbolos y espacios."""
...
def _pick_col(cols_norm: list[str], cols_orig: list[str], candidates: list[str]):
    cand_norm = [_norm_header(c) for c in candidates]
    for cand in cand_norm:
        if cand in cols_norm:
            return cols_orig[cols_norm.index(cand)]
    for i, col in enumerate(cols_norm):
        for cand in cand_norm:
            if cand and (col == cand or col.startswith(cand + " ") or cand in col):
                return cols_orig[i]
    return None
...
def _looks_like_full_sheet(df: pd.DataFrame) -> bool:
    if df is None or df.empty:
        return False
    cols_norm = [_norm_header(c) for c in df.columns.tolist()]
    has_sku = any(c in cols_norm for c in ["sku", "sku ml", "cod sku"])
    has_qty = any(c in cols_norm for c in ["unidades", "cantidad", "cant", "qty", "cantidad total"])
    has_desc = any(c in cols_norm for c in ["descripcion", "producto", "articulo", "detalle"])
    return has_sku and has_qty and has_desc


def read_full_excel(file) -> pd.DataFrame:
    """
    Lee Excel FULL nuevo y formatos anteriores. Devuelve columnas internas:
    codigo_ml, codigo_universal, sku_ml, title, qty_required, area, nro,
    etiquetar, es_pack, instruccion, vence, dia, hora, sheet.
    """
    ...
    for sh in xls.sheet_names:
        ...
        if not _looks_like_full_sheet(df):
            try:
                raw = pd.read_excel(xls, sheet_name=sh, header=None, dtype=str)
                for header_row in range(min(8, len(raw))):
                    tmp = raw.iloc[header_row + 1:].copy()
                    tmp.columns = raw.iloc[header_row].tolist()
                    tmp = tmp.dropna(how="all")
                    if _looks_like_full_sheet(tmp):
                        df = tmp
                        break
            except Exception:
                pass
        ...
        cols_norm = [_norm_header(c) for c in cols_orig]

        area_col = _pick_col(cols_norm, cols_orig, ["area", "area.", "área", "zona", "ubicacion", "ubicación"])
        nro_col = _pick_col(cols_norm, cols_orig, ["nº", "n°", "nro", "numero", "número", "num", "#", "n"])
        codigo_ml_col = _pick_col(cols_norm, cols_orig, ["codigo ml", "código ml", "cod ml", "mlc", "publicacion", "publicación"])
        codigo_univ_col = _pick_col(cols_norm, cols_orig, ["codigo universal", "código universal", "cod universal", "ean", "codigo ean", "código ean", "upc", "gtin"])
        sku_col = _pick_col(cols_norm, cols_orig, ["sku", "sku ml", "sku_ml", "codigo", "código", "cod", "ubc", "cod sku"])
        title_col = _pick_col(cols_norm, cols_orig, ["descripcion", "descripción", "articulo", "artículo", "producto", "detalle", "artículo producto", "articulo producto"])
        qty_col = _pick_col(cols_norm, cols_orig, ["unidades", "cantidad", "qty", "cant", "cant.", "cantidad total"])
        etiquetar_col = _pick_col(cols_norm, cols_orig, ["identificacion", "identificación", "etiquetar", "etiqueta", "etiq"])
        pack_col = _pick_col(cols_norm, cols_orig, ["es pack", "pack", "es_pack", "espack"])
        instr_col = _pick_col(cols_norm, cols_orig, ["instruccion", "instrucción", "instrucciones", "obs", "observacion", "observación", "nota", "notas"])
        vence_col = _pick_col(cols_norm, cols_orig, ["vence", "vencimiento", "fecha vence", "fecha vencimiento"])
        dia_col = _pick_col(cols_norm, cols_orig, ["dia", "día"])
        hora_col = _pick_col(cols_norm, cols_orig, ["hora"])

        # Fallback específico SOLO para el formato nuevo:
        # Area., Nº, Código ML, Código Universal, SKU, Descripción, Unidades, Identificación, Vence, Dia, Hora
        # Importante: no aplicar este fallback a formatos antiguos, porque en Full 2403 no existe
        # Código Universal y se terminaba copiando por error el Código ML como Código Universal.
        is_new_full_format = (
            len(cols_orig) >= 11
            and _norm_header(cols_orig[0]) in ["area", "area."]
            and _norm_header(cols_orig[1]) in ["nº", "n°", "nro", "numero", "número"]
            and _norm_header(cols_orig[2]) in ["codigo ml", "código ml", "cod ml"]
            and _norm_header(cols_orig[3]) in ["codigo universal", "código universal", "cod universal"]
            and _norm_header(cols_orig[4]) == "sku"
        )

        if is_new_full_format:
            if area_col is None:
                area_col = cols_orig[0]
            if nro_col is None:
                nro_col = cols_orig[1]
            if codigo_ml_col is None:
                codigo_ml_col = cols_orig[2]
            if codigo_univ_col is None:
                codigo_univ_col = cols_orig[3]
            if sku_col is None:
                sku_col = cols_orig[4]
            if title_col is None:
                title_col = cols_orig[5]
            if qty_col is None:
                qty_col = cols_orig[6]
            if etiquetar_col is None:
                etiquetar_col = cols_orig[7]
            if vence_col is None:
                vence_col = cols_orig[8]
            if dia_col is None:
                dia_col = cols_orig[9]
            if hora_col is None:
                hora_col = cols_orig[10]
        ...
        for _, r in df.iterrows():
            ...
            qty = _qty_to_int(r.get(qty_col, "")) if qty_col else 0
            if qty <= 0:
                continue

            all_rows.append({
                "sheet": sh,
                "codigo_ml": _safe_str(r.get(codigo_ml_col, "")) if codigo_ml_col else "",
                "codigo_universal": _safe_str(r.get(codigo_univ_col, "")) if codigo_univ_col else "",
                "sku_ml": sku,
                ...
                "dia": _safe_str(r.get(dia_col, "")) if dia_col else "",
                "hora": _safe_str(r.get(hora_col, "")) if hora_col else "",
            })
...
def upsert_full_batch_from_df(df: pd.DataFrame, batch_name: str):
    ...
    for _, r in df.iterrows():
        ...
        for k in ["codigo_ml", "codigo_universal", "etiquetar", "es_pack", "instruccion", "vence", "dia", "hora"]:
            v = _safe_str(r.get(k, ""))
    ...
    for sku, a in agg.items():
        ...
        c.execute("""
            ...
            (batch_id, sku_ml, title, areas, nros, codigo_ml, codigo_universal, etiquetar, es_pack, instruccion, vence, dia, hora, qty_required, qty_checked, status, updated_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            batch_id, sku, a["title"], areas_txt, nros_txt,
            a.get("codigo_ml", ""), a.get("codigo_universal", ""),
            a.get("etiquetar", ""), a.get("es_pack", ""), a.get("instruccion", ""), a.get("vence", ""),
            a.get("dia", ""), a.get("hora", ""),
...
def page_full_supervisor(inv_map_sku: dict):
    ...
    c = conn.cursor()
    c.execute("""
        SELECT sku_ml, COALESCE(NULLIF(title,''),''),
               qty_required, COALESCE(qty_checked,0),
               COALESCE(codigo_ml,''), COALESCE(codigo_universal,''),
               COALESCE(etiquetar,''), COALESCE(es_pack,''), COALESCE(instruccion,''),
               COALESCE(vence,''), COALESCE(dia,''), COALESCE(hora,'')
        FROM full_batch_items
        WHERE batch_id=? AND sku_ml=?
    """, (batch_id, sku_cur))
    row = c.fetchone()
    ...
    sku_db, title_db, qty_req, qty_chk, codigo_ml_db, codigo_universal_db, etiquetar_db, es_pack_db, instruccion_db, vence_db, dia_db, hora_db = row
    ...
    codigo_ml_txt = str(codigo_ml_db or "").strip() or "-"
    codigo_universal_txt = str(codigo_universal_db or "").strip() or "-"
    ...
    dia_txt = str(dia_db or "").strip() or "-"
    hora_txt = str(hora_db or "").strip() or "-"
    ...
    st.markdown(
        ...
        <div class="hero2">
            ...
            <div class="meta">ML: {codigo_ml_txt} • Universal: {codigo_universal_txt}<br/>Identificación: {etiquetar_txt} • ES PACK: {es_pack_txt}<br/>Instrucción: {instruccion_txt} • Vence: {vence_txt} • Día: {dia_txt} • Hora: {hora_txt}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
def page_full_admin():
    ...
    c.execute("""
        SELECT sku_ml, COALESCE(NULLIF(title,''),''), codigo_ml, codigo_universal, qty_required, qty_checked,
               (qty_required - qty_checked) as pendiente,
               status, updated_at, areas, nros, etiquetar, vence, dia, hora
        FROM full_batch_items
        WHERE batch_id=?
        ORDER BY status, CAST(sku_ml AS INTEGER), sku_ml
    """, (batch_id,))
    rows = c.fetchall()
    df = pd.DataFrame(rows, columns=["SKU", "Artículo", "Código ML", "Código Universal", "Solicitado", "Acopiado", "Pendiente", "Estado", "Actualizado", "Áreas", "Nros", "Identificación", "Vence", "Día", "Hora"])
